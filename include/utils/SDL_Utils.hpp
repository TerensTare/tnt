...
namespace std
{
    ...
    {
        void operator()(TTF_Font *_Ptr) const noexcept { TTF_CloseFont(_Ptr); }
    };

    template <> struct default_delete<Mix_Chunk>
    {
        void operator()(Mix_Chunk *_Ptr) const noexcept { Mix_FreeChunk(_Ptr); }
    };

    template <> struct default_delete<Mix_Music>
    {
        void operator()(Mix_Music *_Ptr) const noexcept { Mix_FreeMusic(_Ptr); }
    };
} // namespace std
