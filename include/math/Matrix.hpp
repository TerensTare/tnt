...
// TODO: add specializations for Matrix<T, 1,x> | Matrix<T, x, 1> | Matrix<T, 1,
// 1> so that the class wouldn't use two arrays.
...
// TODO: add conversion operator for Vector, SDL_Point, SDL_FPoint, std::pair,
// etc.
namespace tnt
{
    ...
    class Matrix
    {
      public:
        ...
        std::tuple<Matrix, double, int> power_iterator(unsigned rowNum,
                                                       double tolerance);
    ...
    namespace exp
    {
        ...
        template <typename T, std::size_t W> class Matrix<T, W, 1>
        {
          private:
            std::array<T, W> data;
        };

        template <typename T> class Matrix<T, 1, 1>
        {
          private:
            T data;
        };

        template <std::size_t W, std::size_t H> class Matrix<void, W, H>
        {
