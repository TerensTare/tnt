...
inline float EaseIn(float time, float begin, float change, float duration)
{
	...
	float s = p / 4;
	time -= 1;
	return -(change * powf(2, 10 * time) * sinf((time * duration - s) * (2 * PI.first) / p)) + begin;
}
...
inline float EaseOut(float time, float begin, float change, float duration)
{
	...
	float p = duration * .3f;
	float s = p / 4;
	return (change * powf(2, -10 * time) * sinf((time * duration - s) * (2 * PI.first) / p) + change + begin);
}
...
inline float EaseInOut(float time, float begin, float change, float duration)
{
	...
	{
		time -= 1;
		return -.5f * (change * powf(2, 10 * time) * sinf((time * duration - s) * (2 * PI.first) / p)) + begin;
	}
	time -= 1;
	return change * powf(2, -10 * time) * sinf((time * duration - s) * (2 * PI.first) / p) * .5f + change + begin;
}
} // namespace elastic
...
inline float EaseIn(float time, float begin, float change, float duration)
{
	return -change * cosf(time / duration * (PI.first / 2)) + begin;
}

inline float EaseOut(float time, float begin, float change, float duration)
{
	return -change * sinf(time / duration * (PI.first / 2)) + begin;
}
