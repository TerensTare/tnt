...
namespace tnt
{
    ...
    inline const auto SizeOfArray =
        [](Array arr) -> decltype(std::extent<decltype(arr)>::value) {
        return std::extent<decltype(arr)>::value;
    };

    template <class T, std::size_t K>
    constexpr std::size_t get_size(T (&)[K]) noexcept
    {
        return K;
    }

    template <typename... Ts> inline const auto sum(Ts... ts)
    {
        return (ts + ... + 0);
    }

    template <typename... Ts> inline const auto product(Ts... ts)
    {
        return (ts * ... * 1);
    }
    ...
    template <typename T>
    inline void quick_remove_at(std::vector<T> &v,
    ...
    template <typename C, typename T> void insert_sorted(C &v, const T &item)
    {
    ...
    // TODO: edit this
    template <typename T> inline T *void_cast(void *data)
    {
