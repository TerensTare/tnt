...
namespace tnt::ImGui
{
    struct global_config
    {
        ...
        SDL_Texture *font_tex;
            ...
            .font_size = 14,
            .default_font =
                std::string{SDL_GetBasePath()}.append("zeldadxt.ttf")}};
    ...
    } * progress_bar_cfg{new progress_bar_config{
            .w            = 100,
            .h            = 20,
            .idle_color   = SDL_Color{100, 100, 100, 255},
            .filled_color = SDL_Color{200, 100, 100, 255}}};
    ...
    {
        static unsigned char arr[4]{global_cfg->bg.r, global_cfg->bg.g,
                                    global_cfg->bg.b, global_cfg->bg.a};
        return arr;
    }
    ...
    {
        static unsigned char arr[4]{slider_cfg->color.r, slider_cfg->color.g,
    ...
    {
        static unsigned char arr[4]{
            slider_cfg->thumb_idle_color.r, slider_cfg->thumb_idle_color.g,
    ...
    {
        static unsigned char arr[4]{hslider_cfg->thumb_active_color.r,
    ...
    auto draw_rect = [](Window *win, SDL_Rect const &rect,
                        SDL_Color const &color) -> void {
    ...
    auto draw_char = [](Window *win, char letter, int x, int y,
    ...
    auto draw_text = [](Window *win, std::string_view text, int x, int y,
                        int size = global_cfg->font_size) -> void {
    ...
    void make_context(Window *win) noexcept
    {
        ...
        global_cfg->font_tex = IMG_LoadTexture(
            win->getRenderer(), global_cfg->default_font.c_str());
    ...
    void destroy_context() noexcept
    {
        ...
        delete progress_bar_cfg;
        progress_bar_cfg = nullptr;
    ...
    int slider_int(Window *win, int id, int x, int y, int min_, int max_,
                   int *value) noexcept
    {
        int ypos{((slider_cfg->h - slider_cfg->thumb_h) * (*value - min_)) /
                 (max_ - min_)};
    ...
    int slider_float(Window *win, int id, int x, int y, float min_, float max_,
                     float *value) noexcept
    {
        int ypos{static_cast<int>(
            ((slider_cfg->h - slider_cfg->thumb_h) * (*value - min_)) /
            (max_ - min_))};
        int offset{slider_cfg->w - slider_cfg->thumb_w};
        ...
        if (context->active == id)
        {
            ...
            if (mousePos > (slider_cfg->h - 1))
                mousePos = (slider_cfg->h - 1);
            if (float v{static_cast<float>((mousePos * max_) /
                                           (slider_cfg->h - 1))};
                v != *value)
            {
                *value = v;
                    ...
                    unsigned char max_, unsigned char *value) noexcept
    {
        ...
        if (context->active == id)
        {
            ...
            if (mousePos > (slider_cfg->h - 1))
                mousePos = (slider_cfg->h - 1);
            unsigned char v{static_cast<unsigned char>((mousePos * max_) /
                                                       (slider_cfg->h - 1))};
            if (v != *value)
            {
                *value = v;
    ...
    int hslider_int(Window *win, int id, int x, int y, int min_, int max_,
                    int *value) noexcept
    {
        int xpos{((hslider_cfg->w - hslider_cfg->thumb_w) * (*value - min_) /
                  (max_ - min_))};
    ...
    int hslider_float(Window *win, int id, int x, int y, float min_, float max_,
                      float *value) noexcept
    {
        int xpos{static_cast<int>(
            ((hslider_cfg->w - hslider_cfg->thumb_w) * (*value - min_)) /
            (max_ - min_))};
        int offset{hslider_cfg->h - hslider_cfg->thumb_h};
...
void tnt_imgui_begin() noexcept { tnt::ImGui::context->hot = 0; }
