...
void tnt::HorizontalCamera::Move(float x, [[maybe_unused]] float y)
{
    bounds.x = bounds.x + x;
}
...
void tnt::HorizontalCamera::MoveTo(float x, [[maybe_unused]] float y)
{
    bounds.x = x;
}

void tnt::HorizontalCamera::MoveTo(tnt::Vector const &v)
{
    bounds.x = v.x;
}

void tnt::HorizontalCamera::CenterTo(float x, [[maybe_unused]] float y)
{
    bounds.x = (x - (bounds.w / 2));
}
...
SDL_FRect tnt::HorizontalCamera::FBounds() const noexcept { return SDL_FRect{bounds}; }

SDL_Rect tnt::HorizontalCamera::Bounds() const noexcept
{
    return SDL_Rect{(int)bounds.x, (int)bounds.y, (int)bounds.w, (int)bounds.h};
}
